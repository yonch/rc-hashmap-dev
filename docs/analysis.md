...
- Missing lifetime tie between returned references and the `Ref`
  ...
  >> yes please document this. note we have not yet implemented so no need to change code
...
- Interior mutability + aliasing during `Ref::drop`
  ...
  >> Let's do these mitigations except the optional one.
...
- Reentrancy during drops
  ...
  >> The slotmap API for removal returns the removed value. So the removal code can finish removing the entries from the index and slotmap first, and only then drop the value. This means that there is no reentrant manipulation of entries and index, and they are left consistent every time code outside rc-hashmap is allowed to run (like 'K'/'V' drops). Please document this.
...
- Keepalive-on-drop and zero-ref entries
  ...
  >> Let's do the strong invariant approach. Please document this.
...
- Refcount robustness and panics in `Drop`
  ...
  >> Instead of Clone() for Ref, let's provide a try_clone() that returns Result<Ref, Error> on overflow. This way the user can handle the error instead of panicking. Please document this. Drop should be sound and never panic since now we know that Refcount cannot overflow.
...
- State model of entries vs. index
  ...
  >> A key could either be indexed+live or gone, because we do not have an API to create unindexed-but-live entries. Please document this.
...
- API shape for insertion and removal
  ...
  >> Yes just the first behavior -- all creation APIs return a Ref. Please document this.
...
- Index removal on `Ref::drop` when index was already removed
  ...
  >> On Ref::drop, we are guaranteed the invariant that the slot is in the index, so we can always remove it -- it is never "not present". Please document this.
...
>> Document the design choice not to support Deref for Ref so the borrow checking on RcHashMap ensures no aliasing mutable access to V.
...
- Index vs. liveness semantics may surprise
  ...
  >> Do we have a remove(key) API? If so let's remove that. The only way to remove is via Ref::drop. Please document this design choice. We deliberately do not have a remove API.
...
>> Yes see comment above on try_clone() and not supporting Clone().
...
- Codify SlotMap invariants relied upon: removal never relocates other entries; address stability across removals; no shrinking while `&V` exists. >> Yes let's do this
- Define the entry state machine (indexed+live, unindexed+live, gone) and which APIs observe which states.
- Strengthen keepalive precondition: either “no zero-ref entries can exist” (preferred), or track total live refs and only keepalive when `live_refs > 0`. >> yes the preferred
...
>> yes, forbid zero-ref entries by construction
...
>> there is no detached functionality, index is tied to liveness
